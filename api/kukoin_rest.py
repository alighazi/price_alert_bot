...
class RestApiKukoin:    
    ...
    PATH_CANDLESTICK_DATA = "api/v1/market/candles?type=1month&symbol=BTC-USDT"
    ...
    @cache("kukoin.getath", 86400, [1,2])
    def get_ath(self, fsym, tsym):
        #set up the query params
        query_params = {}
        query_params["symbol"] = (fsym+"-"+tsym).upper()  #BTC-USDT
        query_params["interval"] = "1month" # 1M = 1 month
        ...
        highest_value = 0.0
        highest_datetimeseconds = 0
        array = r.json()["data"]
        for c in array:
            print(float(c[3]) )
            if float(c[3]) > highest_value:
                highest_value = float(c[2])
                highest_datetimemilliseconds = int(c[0])

        return highest_value, highest_datetimeseconds*1000

    @cache("kukoin.exchangeinfo", 86400)
